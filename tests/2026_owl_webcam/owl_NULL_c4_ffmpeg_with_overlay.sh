...
RESTART_ATTEMPT=0
PIPE_PATH="/tmp/owl_camera_null.mkv"
...
setup_pipe() {
    ...
    cat "$PIPE_PATH" > /dev/null &
    DRAIN_PID=$!
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] Drain started (PID $DRAIN_PID) on $PIPE_PATH" | tee -a "$LOG_FILE"
}
...
run_ffmpeg() {
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] Starting FFmpeg recording to pipe (null drain)" | tee -a "$LOG_FILE"
...
main() {
    ...
    while true; do
        ...
        # Ensure drain is still running (it exits when ffmpeg closes the pipe)
        if ! kill -0 "$DRAIN_PID" 2>/dev/null; then
            echo "[$(date '+%Y-%m-%d %H:%M:%S')] Drain exited, restarting..." | tee -a "$LOG_FILE"
            cat "$PIPE_PATH" > /dev/null &
            DRAIN_PID=$!
        fi
