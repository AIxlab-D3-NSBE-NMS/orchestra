...
run_ffmpeg() {
    ...
    ffmpeg -use_wallclock_as_timestamps 1 -f v4l2 -input_format nv12 -i /dev/video0 -video_size 1920x1080 -thread_queue_size 64 -framerate 30 -bf 0 -c:v h264_nvenc -preset p1 -y "$output_file" 2>&1 | tee -a "$LOG_FILE"
    return $?
}
main() {
